...
def setup_production_environment():
    ...
    production_env = {
        ...
        'USE_MOCK': 'true',     # Mock mode until user account is funded
...
def validate_production_readiness():
    ...
    try:
        with open('libs/drift/swift_envelope.py', 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
...
async def deploy_dual_bot_system():
    ...
    try:
        ...
        sys.path.insert(0, '.')
        from orchestrator.master import Orchestrator, BotState
        ...
        # Check for required bot failures
        failed_bots = []
        for bot in orchestrator.bots:
            if bot.spec.required and bot.state == BotState.ERROR:
                failed_bots.append(bot.spec.role)
                exit_code = 1
        ...
        if exit_code == 0:
            ...
            logger.error("🚨 Deployment aborted due to failures")
            if failed_bots:
                logger.error(f"Required bots failed: {', '.join(failed_bots)}")
            else:
                logger.error("Orchestrator encountered unexpected errors")

        # Ensure we propagate failure to shell/CI
        if exit_code != 0:
