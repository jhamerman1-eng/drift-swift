...
# Import OrderType for Oracle order handling
...
async def test_oracle_order_handling():
    ...
    try:
        from driftpy.swift.order_subscriber import SwiftOrderSubscriberConfig
        import json
        ...
        # Create and connect DriftpyClient
        drift_client = DriftpyClient(
            rpc_url=config.get_rpc_url(),
            wallet_secret_key=keypair.secret(),
            env=config.get_drift_env()
        )
        await drift_client.connect()  # Must connect before accessing drift_client property

        logger.info("✅ DriftClient created and connected for Oracle order testing")

        # Test the concept
        if drift_client.drift_client is None:
            raise ValueError("Drift client not properly initialized")

        swift_config = SwiftOrderSubscriberConfig(
            drift_client=drift_client.drift_client,  # Use the underlying DriftClient from DriftpyClient
            keypair=keypair,
            drift_env="devnet",
