...
WORKING VERSION - Handles existing user accounts properly
"""
import json
import asyncio
import time
...
class RealDriftClient:
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Initialize user account - FIXED: handle existing accounts properly
            print("👤 Initializing user account...")
            try:
                # Always try to get existing user account first
                self.drift_user = DriftUser(
                    self.drift_client,
                    keypair.pubkey(),
                )
                await self.drift_user.subscribe()
                print("✅ Existing user account loaded")
            except Exception as e:
                print(f"   No existing account found, creating new one...")
                try:
                    ...
                    # Try to load existing account again (might have been created by another process)
                    try:
                        self.drift_user = DriftUser(
                            self.drift_client,
                            keypair.pubkey(),
                        ...
                        await self.drift_user.subscribe()
                        print("✅ User account loaded after retry")
                    except Exception as retry_error:
                        print(f"❌ Failed to load user account: {retry_error}")
                        return False
    ...
    async def get_market_info(self, market_index: int = 0):
        ...
        try:
            ...
            print(f"📊 Market {market_index}: {market.name}")
...
async def main():
    ...
    try:
        ...
        if response in ["yes", "y"]:
            ...
            if result:
                print(f"🎉 SUCCESS! Real order placed on beta.drift.trade!")
