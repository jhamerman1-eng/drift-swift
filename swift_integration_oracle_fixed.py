...
class OracleAwareSwiftOrderSubscriber:
    ...
    async def _process_order(self, order: Dict, accept_sanitized: bool):
        ...
        try:
            ...
            # 🔧 FIXED ORACLE DETECTION: Check by characteristics, not just type
            # Oracle orders may be misclassified as Market orders by DriftPy
            
            # Detect Oracle orders by characteristics
            has_oracle_offset = getattr(order_params, 'oracle_price_offset', None) is not None
            has_auction_prices = (getattr(order_params, 'auction_start_price', None) is not None or 
            ...
            logger.info(f"      Type: {getattr(order_params, 'order_type', 'Unknown')}")
            logger.info(f"      Price: {getattr(order_params, 'price', 0)}")
            logger.info(f"      Oracle Offset: {getattr(order_params, 'oracle_price_offset', None)}")
            logger.info(f"      Auction Start: {getattr(order_params, 'auction_start_price', None)}")
            logger.info(f"      Auction End: {getattr(order_params, 'auction_end_price', None)}")
            logger.info(f"      Oracle by type: {is_oracle_by_type}")
            logger.info(f"      Oracle by characteristics: {is_oracle_by_characteristics}")
            
            # Process Oracle orders (detected by type OR characteristics)
            if is_oracle_by_type or is_oracle_by_characteristics:
                self.stats["oracle_orders_received"] += 1
                
                # Oracle orders have price=0 - this is CORRECT!
                # They use oracle_price_offset or auction_prices instead
                if getattr(order_params, 'oracle_price_offset', None) is not None or has_auction_prices:
                    logger.info("🎯 Oracle Order Received (VALID):")
                    logger.info(f"   Market: {getattr(order_params, 'market_index', 'Unknown')}")
                    logger.info(f"   Direction: {getattr(order_params, 'direction', 'Unknown')}")
                    logger.info(f"   Size: {getattr(order_params, 'base_asset_amount', 0) / BASE_PRECISION:.4f}")
                    logger.info(f"   Oracle Offset: {getattr(order_params, 'oracle_price_offset', None)}")
                    if has_auction_prices:
                        start_price = getattr(order_params, 'auction_start_price', None)
                        end_price = getattr(order_params, 'auction_end_price', None)
                        start_formatted = start_price / PRICE_PRECISION if start_price else None
                        end_formatted = end_price / PRICE_PRECISION if end_price else None
                        logger.info(f"   Auction Prices: ${start_formatted:.2f} -> ${end_formatted:.2f}")
                    
                    # Estimate effective price
                    market_index = getattr(order_params, 'market_index', None)
                    oracle_offset = getattr(order_params, 'oracle_price_offset', None)
                    if market_index in self.estimated_oracle_prices and oracle_offset:
                        oracle_price = self.estimated_oracle_prices[market_index]
                        effective_price = oracle_price + (oracle_offset / PRICE_PRECISION)
                    ...
                    logger.warning("⚠️ Oracle order missing oracle_price_offset AND auction_prices")
                    self.stats["orders_filtered"] += 1
                    return
                    
            # Handle Market orders (original logic)
            elif getattr(order_params, 'price', 0) > 0:
                ...
                logger.info(f"   Direction: {getattr(order_params, 'direction', 'Unknown')}")
                logger.info(f"   Size: {getattr(order_params, 'base_asset_amount', 0) / BASE_PRECISION:.4f}")
                logger.info(f"   Price: ${getattr(order_params, 'price', 0) / PRICE_PRECISION:.4f}")
                ...
                logger.warning(f"⚠️ Filtered order - Type: {getattr(order_params, 'order_type', 'Unknown')}, Price: {getattr(order_params, 'price', 0)}")
                self.stats["orders_filtered"] += 1
                return
