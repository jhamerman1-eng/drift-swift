#!/usr/bin/env python3
"""
Trend Bot - Using Correct DLOB API for Real Price Data
Uses https://dlob.drift.trade for live orderbook and price data
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('trend_bot_dlob_api.log', encoding='utf-8'),
...
class DLOBAPIClient:
    """Client for DLOB API - https://dlob.drift.trade"""
    ...
    async def get_mid_and_oracle_price(self, market="SOL-PERP"):
        """Get mid price and oracle price from DLOB"""
        try:
            l2_data = await self.fetch_l2_orderbook(market, depth=1, include_oracle=True)
            
            if not l2_data:
                logger.warning("No L2 data received")
            ...
            if "bids" in l2_data and l2_data["bids"]:
                best_bid = float(l2_data["bids"][0]["price"])
                logger.debug(f"Best bid: ${best_bid:.4f}")
            
            if "asks" in l2_data and l2_data["asks"]:
                best_ask = float(l2_data["asks"][0]["price"])
            ...
            oracle_price = None
            if "oracleData" in l2_data and l2_data["oracleData"]:
                oracle_price = float(l2_data["oracleData"]["price"])
                logger.debug(f"Oracle price: ${oracle_price:.4f}")
...
async def run_trend_bot_with_dlob():
    """Run the trend bot using DLOB API for real-time data"""
    try:
        logger.info("Starting Trend Bot with DLOB API for Real Price Data")
        logger.info("=" * 70)
        ...
        logger.info("Data Source: DLOB API (https://dlob.drift.trade)")
        logger.info("=" * 70)
        ...
        logger.info("Starting trend analysis loop with DLOB API...")
        ...
        while True:
            try:
                # Get REAL price data from DLOB API
                current_price = None
                mid_price = None
                oracle_price = None
                
                try:
                    # Get live price data from DLOB
...
if __name__ == "__main__":
    asyncio.run(run_trend_bot_with_dlob())
