#!/usr/bin/env python3
"""
Create Drift User Account and Place IMMEDIATE LIVE SELL ORDER
"""
...
async def create_account_and_sell():
    """Create Drift user account and place immediate live sell order"""
    logger.info("STARTING DRIFT USER ACCOUNT CREATION AND LIVE SELL ORDER")
    ...
    try:
        ...
        try:
            # Use the correct driftpy method to initialize user account
            await client.drift_client.initialize_user()
            logger.info("✅ User account created successfully!")
        except Exception as e:
            logger.warning(f"User account creation failed (may already exist): {e}")
            logger.info("Account may already exist, continuing with order placement...")
        ...
        logger.error(f"ERROR in create account and sell: {e}")
        import traceback
        logger.error(f"Traceback: {traceback.format_exc()}")

if __name__ == "__main__":
    asyncio.run(create_account_and_sell())
