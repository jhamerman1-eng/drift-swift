...
class JITEngine:
    def __init__(self, client, cfg: Dict[str, Any], market: str, max_errors: int = 5):
        ...
        self._microprice_calc = MicropriceCalculator(self.config.get("obi_weights") or {})
    ...
    async def step(self) -> Optional[QuoteDecision]:
        ...
        try:
            ...
            self._metrics.record_decision(decision, step_duration)
            JIT_TICK_TO_QUOTE.labels("jit").observe(step_duration)
    ...
    async def _gather_market_data(self) -> Optional[MarketData]:
        ...
        try:
            ...
            # Check if orderbook is None
            if not orderbook:
                logger.warning("Received None orderbook from client")
                return None
            ...
            if best_bid is None or best_ask is None:
                logger.warning(f"Invalid best bid/ask: bid={best_bid}, ask={best_ask}")
                return None
            if best_bid <= 0 or best_ask <= 0 or best_ask <= best_bid:
                # invalid or crossed book
                logger.warning(f"Invalid or crossed book: bid={best_bid}, ask={best_ask}")
                return None
            ...
            logger.error(f"Client missing get_orderbook method: {e}")
            return None
        except Exception as e:
