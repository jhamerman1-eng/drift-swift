#!/usr/bin/env python3
"""
Trend Bot - Beta.Drift.Trade Runner with REAL Price Data
Uses the same RealOrderbookFetcher as the MM bot to get live price data
"""
...
sys.path.insert(0, str(Path(__file__).parent / "drift-bots"))
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('trend_bot_beta_real_data.log', encoding='utf-8'),
...
async def run_trend_bot_with_real_data():
    """Run the trend bot with REAL price data from Drift"""
    try:
        logger.info("Starting Trend Bot with REAL Price Data on Beta.Drift.Trade")
        logger.info("=" * 70)
        ...
        logger.info("Data Source: REAL Drift Oracle + Orderbook")
        logger.info("=" * 70)
        ...
        # Initialize REAL orderbook fetcher (same as MM bot)
        try:
            from drift_bots.libs.drift.real_orderbook import RealOrderbookFetcher
            orderbook_fetcher = RealOrderbookFetcher(client.drift_client)
            await orderbook_fetcher.initialize()
            logger.info("Real orderbook fetcher initialized successfully")
            real_data_available = True
        except Exception as e:
            logger.warning(f"Real orderbook fetcher failed: {e}")
        ...
        while True:
            try:
                ...
                if real_data_available and orderbook_fetcher:
                    try:
                        # Get live orderbook from Drift (same as MM bot)
                        live_orderbook = await orderbook_fetcher.get_live_orderbook(0)  # SOL-PERP market
                        current_price = live_orderbook.oracle_price
                        real_price_count += 1
                        
                        logger.info(f"REAL PRICE DATA: ${current_price:.4f} (Oracle: ${live_orderbook.oracle_price:.4f})")
                        logger.info(f"Orderbook: {len(live_orderbook.bids)} bids, {len(live_orderbook.asks)} asks")
                        logger.info(f"Spread: {live_orderbook.spread_bps:.2f} bps")
                        
                        # Add price to our tracking
                        prices.append(current_price)
                        
                    except Exception as e:
                        logger.warning(f"Real price fetch failed: {e}")
                ...
                if iteration_count % 5 == 0:
                    ...
                    logger.info(f"Real Price Data: {real_price_count}/{iteration_count} iterations")
                    logger.info("=" * 50)
...
if __name__ == "__main__":
    asyncio.run(run_trend_bot_with_real_data())
