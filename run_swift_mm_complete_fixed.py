...
class CompleteSwiftMMBot:
    ...
    async def _initialize_jit_client(self):
        ...
        try:
            ...
            self.jit_enabled = jit_config.get("enabled", False)
    ...
    async def _place_order_via_sidecar(self, side: str, price: float, size: float) -> Optional[str]:
        """Place order with JIT routing support - US-JIT-005"""
        try:
            if self.degraded_mode:
            ...
            # FIXED: Re-enable Smart Routing with JIT → Swift → DriftPy fallback chain
            ...
            # Priority 3: DriftPy Direct (fallback)
            # 
    ...
    async def _place_order_direct(self, side: str, price: float, size: float) -> Optional[str]:
        ...
        logger.error("🚨 This violates the sidecar-only order flow policy")
        logger.error("🚨 All orders must go through _place_order_via_sidecar()")
        
        # Return None to force proper routing through sidecar
