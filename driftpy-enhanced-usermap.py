...
from driftpy.types import SpotPosition, PerpPosition
...
class EnhancedUserMap:
    ...
    def format_perp_position(self, user: DriftUser, position: PerpPosition) -> FormattedPosition:
        ...
        market_name = "Unknown" if market is None else bytes(market.name).decode('utf-8').strip('\x00')
    ...
    def format_spot_position(self, user: DriftUser, position: SpotPosition) -> FormattedPosition:
        ...
        decimals = 6 if market is None else market.decimals  # Default to 6 decimals if market not found
    ...
    if positions["perp_positions"]:
        ...
        for pos in positions["perp_positions"]:
            try:
                ...
                print(f"Value: ${format_number(abs(pos.position_value))}")
                print(f"Unrealized PnL: ${format_number(pos.pnl)}" if pos.pnl is not None else "Unrealized PnL: N/A")
                print(f"Funding PnL: ${format_number(pos.funding_pnl)}" if pos.funding_pnl is not None else "Funding PnL: N/A")
