...
from drift.swift_sidecar_driver import SwiftSidecarDriver  # type: ignore
from driftpy.drift_client import DriftClient
from drift.swift_envelope import SwiftEnvelopeCreator, SwiftOrderParams  # type: ignore
from drift.swift_receiver import SwiftOrderReceiver, SwiftOrderProcessor, SwiftOrderMessage  # type: ignore
...
class CompleteSwiftMMBot:
    ...
    async def start_swift_receiver(self):
        ...
        try:
            ...
            def sync_callback(order: SwiftOrderMessage) -> None:
                import asyncio
                asyncio.create_task(self._handle_swift_order(order))
            
            await self.swift_receiver.start(sync_callback)
    ...
    async def _get_orderbook(self) -> Optional[Dict]:
        ...
        try:
            if self.drift_client is None:
                return None
            # Get orderbook using the l2 method
            try:
                # Use the DriftPy client's orderbook method
                ob = await self.drift_client.get_l2_orderbook("SOL-PERP", 10)  # type: ignore
            except AttributeError:
                # Fallback for different DriftPy versions
                try:
                    # Alternative method name
                    ob = await self.drift_client.get_orderbook_l2("SOL-PERP", 10)  # type: ignore
                except:
                    # Last resort - basic structure
                    ob = {"bids": [[150.0, 1.0]], "asks": [[151.0, 1.0]]}
            except Exception as e:
    ...
    async def _place_order_via_sidecar(self, side: str, price: float, size: float) -> Optional[str]:
        ...
        try:
            ...
            swift_params = SwiftOrderParams(
                ...
                price=price,
                size=size,
                taker_authority=str(self.keypair.pubkey()) if self.keypair else "",
                sub_account_id=0,
                order_type="limit",
                post_only=True,
                reduce_only=False
            ...
            # Create envelope
            if self.keypair is None:
                logger.error("Keypair not available")
                return None
