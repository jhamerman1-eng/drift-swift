...
# Format 1: Hex encoding with proper signature format
payload_hex = {
    "signedMessage": binascii.hexlify(message_bytes).decode('ascii'),
    "signature": bytes(signature).hex(),  # Keep as hex string
    "marketIndex": 0,
    "marketType": "perp",
...
print("Format 1 (Hex with hex signature):")
print(json.dumps(payload_hex, indent=2))
print()

# Format 2: Base64 encoding with proper message format
payload_base64 = {
    "message": message_bytes.decode('utf-8'),  # Raw string, not base64
...
print("Format 2 (String message with base64 signature):")
print(json.dumps(payload_base64, indent=2))
print()

# Format 3: Raw JSON with proper signature format
payload_json = {
    "order": test_order,
    "signature": bytes(signature).hex(),  # Keep as hex string
    "publicKey": str(keypair.pubkey())
...
print("Format 3 (JSON with hex signature):")
print(json.dumps(payload_json, indent=2))
print()

# Format 4: Try the exact format from our Swift MM bot
payload_swift = {
    "message": {
        "version": "1.0",
        "type": "order",
        "timestamp": 1757812830000,
        "data": {
            "market_index": 0,
            "market_type": "perp",
            "side": "buy",
...
print("Format 4 (Swift MM bot format):")
print(json.dumps(payload_swift, indent=2))
print()
...
for format_name, payload in [("Hex", payload_hex), ("Base64", payload_base64), ("JSON", payload_json), ("Swift", payload_swift)]:
