...
class CompleteSwiftMMBot:
    ...
    async def start_swift_receiver(self):
        ...
        try:
            ...
            swift_urls = [
                ...
                "wss://swift.drift.trade/ws",       # Official Swift WebSocket
                "wss://beta.drift.trade/ws",        # Beta environment WebSocket
            ]
    ...
    async def _place_order_via_swift_api(self, side: str, price: float, size: float) -> str:
        ...
        try:
            ...
            # Create Swift order parameters with compute budget optimization
            if not self.keypair:
            ...
            # Add compute budget optimization for Swift orders
            # Determine order type for optimization (remove undefined context reference)
            order_type = "jit_response"  # Default to JIT response for market making

            # Get compute budget parameters from router
            if hasattr(self, 'execution_router') and self.execution_router:
                swift_params_enhanced = self.execution_router._prepare_swift_order_with_compute_budget(
                    {
                        'market_index': 0,
                        'market_type': 'perp',
                        'side': side,
                ...
                # Apply compute budget to parameters if available
                if 'compute_unit_limit' in swift_params_enhanced:
                    logger.debug(f"🔧 Applied compute budget optimization: {swift_params_enhanced.get('compute_unit_limit')} units @ {swift_params_enhanced.get('compute_unit_price')} micro-lamports")
            else:
                logger.debug("Execution router not available for compute budget optimization")
            ...
            if "SwiftDegradationError" in str(type(e)) or "degraded" in str(e).lower():
                ...
                # Auto-reset circuit breaker after short delay (JIT optimization)
                if "503" in str(e) or "circuit" in str(e).lower():
                    logger.info("🔄 Circuit breaker detected - will auto-reset in 5 seconds for JIT trading")
                    asyncio.create_task(self._reset_circuit_breaker_after_delay(5))
    ...
    async def _reset_circuit_breaker_after_delay(self, delay_seconds: int):
        """🔄 Auto-reset circuit breaker after delay to restore JIT trading"""
        try:
            await asyncio.sleep(delay_seconds)
            
            # Try to make a health check to reset the circuit breaker
            swift_config = self.env_config.get_swift_config()
            sidecar_url = swift_config.get("base_url", "http://localhost:8787")
            
            if sidecar_url.startswith("http://localhost") and httpx is not None:
                async with httpx.AsyncClient(timeout=2.0) as client:
                    health_response = await client.get(f"{sidecar_url}/health")
                    if health_response.status_code == 200:
                        # Reset our sidecar mode cache to allow retrying Swift
                        self.invalidate_sidecar_mode_cache()
                        logger.info("✅ Circuit breaker reset - Swift trading restored")
                    else:
                        logger.warning(f"⚠️ Circuit breaker reset failed: {health_response.status_code}")
            elif httpx is None:
                logger.warning("⚠️ Circuit breaker reset skipped - httpx not available")
        except Exception as e:
            logger.warning(f"⚠️ Circuit breaker reset failed: {e}")
            # Continue with DriftPy fallback
    ...
    async def _place_order_direct(self, side: str, price: float, size: float) -> Optional[str]:
        ...
        try:
            ...
            # Convert to DriftPy types - CRITICAL FIX: use enum values WITH parentheses
            ...
            # Create order parameters - FIXED: All enums now use parentheses to avoid _Constructor errors
            order_params = OrderParams(
                order_type=OrderType.Limit(),  # ✅ FIXED: Added parentheses
                market_type=MarketType.Perp(),  # ✅ FIXED: Use proper MarketType enum instead of 0
                direction=direction,  # ✅ FIXED: Now properly instantiated with parentheses
