...
async def run_hedge_bot():
    ...
    try:
        ...
        # Build client using config path
        ...
        logger.info("🔧 Hedge Configuration:")
        logger.info(f"  📊 Sub-account: {hedge_config.get('hedge', {}).get('sub_account', {}).get('name', 'N/A')}")
        logger.info(f"  🎯 Routes: {[route['venue'] for route in hedge_config.get('hedge', {}).get('route', [])]}")

        # Initialize hedge executor
        from bots.hedge.execution import initialize_hedge_executor
        initialize_hedge_executor(client)
        logger.info("✅ Hedge executor initialized")
