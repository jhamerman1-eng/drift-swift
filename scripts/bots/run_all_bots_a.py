    ...
    try:
        ...
        while RUNNING:
            try:
                ...
                if client is None:
                    logger.error("Trend Bot: Client failed to initialize")
                    break

                # Fix type: state_vars should be Dict[str, float], not Dict[str, None]
                # Initialize with float values (e.g., 0.0) instead of None
                if state_vars["ema_fast"] is None:
                    for k in state_vars:
                        if state_vars[k] is None:
                            state_vars[k] = 0.0
                    for k in state_vars:
                        if state_vars[k] is None:
                            state_vars[k] = 0.0
                ...
                if iteration % 10 == 0:
                    pos_count = len(getattr(position, "positions", []))
                    logger.info(f"📈 Trend Bot: Completed {iteration} iterations, Position: {pos_count}")
